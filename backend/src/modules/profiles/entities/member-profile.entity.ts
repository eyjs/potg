...
export class MemberProfile extends BaseEntity {
  ...
  statusMessage: string | null; // 상태 메시지 "오늘도 용검 들고 갑니다"

  @Column({ type: 'varchar', length: 50, default: 'default' })
  themeId: string; // 적용된 테마 코드

  @Column({ type: 'varchar', nullable: true })
  bgmUrl: string | null; // 배경음악 URL

  @Column({ type: 'varchar', length: 100, nullable: true })
  bgmTitle: string | null; // BGM 제목

  @Column({ type: 'varchar', nullable: true })
  avatarUrl: string | null; // 아바타 이미지

  @Column({ type: 'varchar', length: 50, default: 'default' })
  frameId: string; // 프레임 코드

  @Column({ type: 'varchar', length: 50, nullable: true })
  petId: string | null; // 펫 코드
